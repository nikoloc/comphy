...
//             struct mwc_toplevel *first = wl_container_of(server.active_workspace->floating_toplevels.next, first,
//             link); focus_toplevel(first);
//         }
//     }
//
//     struct mwc_output *o;
...
lock_mgr_init(struct lock_mgr *mgr, struct wl_display *display) {
    mgr->wlr_lock_mgr = wlr_session_lock_manager_v1_create(display);

    mgr->new_lock.notify = handle_new_lock;
    wl_signal_add(&mgr->wlr_lock_mgr->events.new_lock, &mgr->new_lock);
}
