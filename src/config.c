...
config_set_default_needed_params(struct mwc_config *c) {
    ...
    if(c->min_toplevel_size == 0) {
        ...
        wlr_log(WLR_INFO, "min_toplevel_size not specified. using default %ud", c->min_toplevel_size);
    }
    if(c->master_count == 0) {
        c->master_count = 1;
    ...
    if(c->animations && c->animation_curve[0] == 0 && c->animation_curve[1] == 0 && c->animation_curve[2] == 0 &&
...
config_reload() {
    ...
    if(c->blur) {
        ...
        wl_list_for_each(output, &server.outputs, link) {
            ...
            output->blur = wlr_scene_optimized_blur_create(&server.scene->tree, output_box.width, output_box.height);
        ...
        wl_list_for_each(w, &out->workspaces, link) {
            ...
            wl_list_for_each(k, &server.config->keybinds, link) {
                if(k->action == keybind_change_workspace && (uint64_t)k->args == w->index) {
                    k->args = w;
                    k->initialized = true;
                } else if(k->action == keybind_move_focused_toplevel_to_workspace && (uint64_t)k->args == w->index) {
                    k->args = w;
                    k->initialized = true;
                }
            }
        ...
        for(size_t i = 0; i < 4; i++) {
            wl_list_for_each(layer, &(&out->layers.background)[i], link) {
                ...
                wl_list_for_each(b, &server.config->layer_rules.blur, link) {
                    if(!b->condition.has ||
                            regexec(&b->condition.regex, layer->wlr_layer_surface->namespace, 0, NULL, 0) == 0) {
                        wlr_scene_node_for_each_buffer(&layer->scene->tree->node, iter_scene_buffer_apply_blur,
                                (void *)1);
                        found = true;
                        break;
                    }
                ...
                if(!found) {
                    wlr_scene_node_for_each_buffer(&layer->scene->tree->node, iter_scene_buffer_apply_blur, (void *)0);
                }
            }
        }
    }
